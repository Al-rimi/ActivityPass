...
- Maintains a dedicated Python virtual environment in `backend/.venv` (disable with `USE_VENV=false`) and installs dependencies from `backend/requirements.txt`.
...
2. **Install required system packages**: make sure `python3`, `python3-venv`, `pip`, `git`, `npm`, and `sudo` are available. `rsync` is optional but recommended for faster frontend syncs. If you plan to rely on the system interpreter (`USE_VENV=false`), ensure the `python` binary you want is on the `PATH` and already has the project requirements installed.
...
5. **Bootstrap the server**:
   ...
   - Create or reuse the virtualenv at `backend/.venv` and install Python dependencies (skipped when `USE_VENV=false`).
...
6. **Connect the backend to your process manager**:
   - Configure Gunicorn, Daphne, or your preferred ASGI/WSGI server to use the virtualenv in `backend/.venv` (or the system interpreter when `USE_VENV=false`).
...
- Default virtualenv location: `backend/.venv`. Use `$VENV_DIR/bin/python manage.py <command>` (or `backend\.venv\Scripts\python.exe` on Windows) for manual maintenance. Set `USE_VENV=false` to rely on the system interpreter instead; in that mode the deploy script uses `PYTHON_BIN` (default `python3`, override with `PYTHON_BIN=python`).
